...
class Extractor(object):
    ...
    def extract(self):
        self.outwriter.write("# "+repr(self.boardsize)+" x " +
                             repr(self.boardsize) + " Hex state-action-value tuples\n")
        for line in self.src_file:
            if line.startswith("#"):
                continue
            array = line.split()
            if not array[-1].replace('.', '', 1).replace('-', '').isdigit():
                continue
            res = float(array[-1])
            n = len(array) - 1
            for i in range(1, n):
    ...
    def _check_last_move(self, last_move):
        # B[a3][a2:0.3;b3:0.4;a3:0.3], check if a3 if with the largest probability
        array = re.split('\[|\]', last_move)
        move = array[1].strip()  # a3
        move_list = array[3].split(';')  # a2:0.3;
        best_prob, best_move = -1.0, None
        for pair in move_list:
            if ':' not in pair:
                continue
            array2 = pair.split(':')
            m, prob = array2[0].strip(), float(array2[1])
            if best_prob < prob:
                best_prob, best_move = prob, m
        return best_move == move

    def postprocess(self, positionValuesFileName, just_shuffle=False, dithered_threshold=0, dynamic=False):
        ...
        threshold = dithered_threshold
        for line in tt.values():
            # print(line)
            mq, one_count, neg_one_count = line
            n_moves = len(mq)
            f = f_dithered if n_moves <= dithered_threshold else f_normal
            if dynamic and dithered_threshold > 0:
                ret = self._check_last_move(mq[-1])
                f = f_dithered if not ret else f_normal
            for m in mq:
                f.write(m+' ')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_file", type=str,
                        default=None, help='input shf games')
    parser.add_argument("--output_file", type=str,
                        default=None, help='output data file name')
    parser.add_argument("--boardsize", type=int, default=13, help='boardsize')
    parser.add_argument("--post_process", type=str, default='only_shuffle',
                        help="only shuffle or average the value of same game state")
    parser.add_argument("--dithered_threshold", type=int,
                        default=0, help="dither threshold")
    parser.add_argument("--dithered_dynamic", type=bool,
                        default=False, help="dynamic deal with dithered data?")
    ...
    if not args.output_file:
        ...
        exit(1)
    ext = Extractor(args.input_file, args.output_file,
                    boardsize=args.boardsize)
    ext.extract()
    ext.close()
    if args.post_process == "only_shuffle":
        ext.postprocess(args.output_file, just_shuffle=True,
                        dithered_threshold=args.dithered_threshold, dynamic=args.dithered_dynamic)
    else:
